...
for i in range(int(model_width*num_of_slabs/partition_size)-1):
...
##################################################
##### MAKE MULTIPLE SLABS WITH LINEAR PATTERNS
##################################################
instance_list = [i for i in mdl.rootAssembly.instances.keys() if 'trsbar' in i]
instance_list.append('concslab')
instance_list = tuple(instance_list)
mdl.rootAssembly.LinearInstancePattern(direction1=(1.0, 0.0,
    0.0), direction2=(0.0, 1.0, 0.0), instanceList=instance_list, number1=num_of_slabs,
    number2=1, spacing1=model_width, spacing2=model_height)
...
for stbar in stbarInstances:
    ...
    for stbarV in vertices:
        ...
        for conc_instance in {k for k in mdl.rootAssembly.instances.keys() if 'concslab' in k}:
            print(conc_instance)
            concV = mdl.rootAssembly.instances[conc_instance].vertices.findAt(stbarV.pointOn[0])
            # if concV is None:
                # continue
...
stbarCorner, stbarInterior = edgeNameToEdgeArrayFilter(stbarBondHort,
       lambda x: eql(x.pointOn[0][0], 0) or eql(x.pointOn[0][0], model_width*num_of_slabs)
...
for conc_instance in {k for k in mdl.rootAssembly.instances.keys() if 'concslab' in k}:
    for v in mdl.rootAssembly.instances[conc_instance].vertices:
...
while lvl >= 0:
    vertices = []
    for conc_instance in {k for k in mdl.rootAssembly.instances.keys() if 'concslab' in k}:
        vertices.append(mdl.rootAssembly.instances[conc_instance].vertices.getByBoundingBox(yMax=lvl, yMin=lvl))
...
vertices = []
for conc_instance in {k for k in mdl.rootAssembly.instances.keys() if 'concslab' in k}:
    vertices.append(mdl.rootAssembly.instances[conc_instance].vertices.getByBoundingBox(xMax=0, xMin=0))
mdl.rootAssembly.Set(name='NodeSet_xMin', vertices=vertices)
vertices = []
for conc_instance in {k for k in mdl.rootAssembly.instances.keys() if 'concslab' in k}:
    vertices.append(mdl.rootAssembly.instances[conc_instance].vertices.getByBoundingBox(xMax=model_width*num_of_slabs, xMin=model_width*num_of_slabs))
mdl.rootAssembly.Set(name='NodeSet_xMax', vertices=vertices)
vertices = []
for conc_instance in {k for k in mdl.rootAssembly.instances.keys() if 'concslab' in k}:
    vertices.append(mdl.rootAssembly.instances[conc_instance].vertices.getByBoundingBox(zMax=0, zMin=0))
mdl.rootAssembly.Set(name='NodeSet_zMin', vertices=vertices)
vertices = []
for conc_instance in {k for k in mdl.rootAssembly.instances.keys() if 'concslab' in k}:
    vertices.append(mdl.rootAssembly.instances[conc_instance].vertices.getByBoundingBox(zMax=model_depth, zMin=model_depth))
...
for k in mdl.rootAssembly.instances.keys():
    if k.startswith('sbar'):
        ...
        sbarXMax.append(mdl.rootAssembly.instances[k].vertices.getByBoundingBox(xMax=model_width*num_of_slabs, xMin=model_width*num_of_slabs))
...
faces = []
for conc_instance in {k for k in mdl.rootAssembly.instances.keys() if 'concslab' in k}:
    faces.append(mdl.rootAssembly.instances[conc_instance].faces.getByBoundingBox(xMax=0, xMin=0))
mdl.rootAssembly.Set(name='SurfaceSet_xMin', faces=faces)
faces = []
for conc_instance in {k for k in mdl.rootAssembly.instances.keys() if 'concslab' in k}:
    faces.append(mdl.rootAssembly.instances[conc_instance].faces.getByBoundingBox(xMax=model_width*num_of_slabs, xMin=model_width*num_of_slabs))
mdl.rootAssembly.Set(name='SurfaceSet_xMax', faces=faces)
faces = []
for conc_instance in {k for k in mdl.rootAssembly.instances.keys() if 'concslab' in k}:
    faces.append(mdl.rootAssembly.instances[conc_instance].faces.getByBoundingBox(yMax=0, yMin=0))
mdl.rootAssembly.Set(name='SurfaceSet_yMin', faces=faces)
faces = []
for conc_instance in {k for k in mdl.rootAssembly.instances.keys() if 'concslab' in k}:
    faces.append(mdl.rootAssembly.instances[conc_instance].faces.getByBoundingBox(yMax=model_height, yMin=model_height))
mdl.rootAssembly.Set(name='SurfaceSet_yMax', faces=faces)
faces = []
for conc_instance in {k for k in mdl.rootAssembly.instances.keys() if 'concslab' in k}:
    faces.append(mdl.rootAssembly.instances[conc_instance].faces.getByBoundingBox(zMax=0, zMin=0))
mdl.rootAssembly.Set(name='SurfaceSet_zMin', faces=faces)
faces = []
for conc_instance in {k for k in mdl.rootAssembly.instances.keys() if 'concslab' in k}:
    faces.append(mdl.rootAssembly.instances[conc_instance].faces.getByBoundingBox(zMax=model_depth, zMin=model_depth))
